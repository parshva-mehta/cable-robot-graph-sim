"""Phase 3–5 MEKF refiner evaluation: sweeps, diagnostics, and ablations.

Phase 3: refiner behavior (raw vs EKF vs measurement-only), noise sweep, R-sweep.
Phase 4: per-step diagnostics — innovation norm, correction/innovation ratio,
          covariance trace, NEES (innovation chi-square consistency test).
Phase 5: robustness ablations — gating, missing measurements,
          Jacobian update interval sweep, streaming vs batch parity.

Usage:
    python scripts/mekf_refiner_eval.py                         # all phases
    python scripts/mekf_refiner_eval.py --phases 3
    python scripts/mekf_refiner_eval.py --phases 4 --save_plots
    python scripts/mekf_refiner_eval.py --phases 5
...
def _run_raw_gnn(sim, gt_data, extra_data, device):
    """Return (com_err, rot_err, pen_err) from raw GNN."""
...
def _run_ekf(sim, gt_data, extra_data, device, **ekf_kwargs):
    """Return (frames, com_err, rot_err, pen_err) for an EKF rollout."""
    start = _make_start_state(gt_data, len(sim.robot.rigid_bodies), device)
...
def _corrupt_gt(gt_data, sigma_pos, sigma_rot, rng):
    """Return copy of gt_data with Gaussian noise on pos and quat."""
    corrupted = []
    for d in gt_data:
        dc = dict(d)
...
def _measurement_only_error(gt_data, n_rods, device):
    """Error of the measurement-only estimate (GT pose as estimate)."""
    com_errs, rot_errs = [], []
    for i in range(1, len(gt_data)):
        gt_prev = gt_data[i - 1]
        gt_curr = gt_data[i]
        for r in range(n_rods):
            # measurement-only estimate = GT itself → zero error
            gt_pos = torch.tensor(gt_curr["pos"][r * 3 : (r + 1) * 3],
                                  dtype=DEFAULT_DTYPE, device=device).reshape(1, 3)
            gt_quat = torch.tensor(gt_curr["quat"][r * 4 : (r + 1) * 4],
                                   dtype=DEFAULT_DTYPE, device=device).reshape(1, 4)
            # For σ=0, measurement ≡ GT, so error is 0.
            # Here we just verify prediction horizon by comparing step k+1 GT to step k GT
            # (the "measurement-only" baseline has 0 error by definition when GT is the measurement)
            com_errs.append(0.0)
            rot_errs.append(0.0)
    return float(np.mean(com_errs)), float(np.mean(rot_errs))


def phase3_refiner_behavior(model_path, dataset_root, trajs, device,
                             process_noise=1e-6, measurement_noise=1e-1,
                             ratio_target=0.7):
    """Run raw vs EKF comparison; report ratio and check < ratio_target."""
    print("\n=== Phase 3: Refiner behavior ===")
    rows = []
    header = f"{'traj':<10} {'raw_com':>10} {'ekf_com':>10} {'ratio':>8} {'raw_rot':>10} {'ekf_rot':>10}"
    print(header)
    print("-" * len(header))
    ...
    for traj in trajs:
        ...
        _, ekf_com, ekf_rot, ekf_pen = _run_ekf(
        ...
        print(
            f"{traj:<10} {raw_com:>10.4e} {ekf_com:>10.4e} {ratio:>8.3f}"
            f" {raw_rot:>10.4e} {ekf_rot:>10.4e}{tag}"
        )
        rows.append(dict(
            traj=traj, raw_com=raw_com, ekf_com=ekf_com, ratio=ratio,
            raw_rot=raw_rot, ekf_rot=ekf_rot,
        ))
...
def phase3_noise_sweep(model_path, dataset_root, traj, device,
    ...
    """Sweep measurement noise σ and plot EKF error vs σ (expected U-curve)."""
    if sigmas_pos is None:
        sigmas_pos = [0.0, 0.001, 0.005, 0.01, 0.05]

    print(f"\n=== Phase 3: Noise sweep ({traj}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    rng = np.random.default_rng(42)

    print(f"  {'sigma_pos(m)':>12} {'raw_com':>12} {'ekf_com':>12} {'meas_com':>12}")
    raw_com_base = None
    ekf_coms = []
    raw_com_ref = None
    ...
    for sigma in sigmas_pos:
        ...
        # measurement-only error = error of using corrupted GT directly
        meas_com_errs = []
...
def phase3_R_sweep(model_path, dataset_root, traj, device,
    ...
    """At fixed noise σ, sweep measurement_noise R; optimal R ≈ σ²."""
    if R_values is None:
        R_values = [1e-4, 3e-4, 1e-3, 3e-3, 1e-2, 3e-2, 1e-1]

    print(f"\n=== Phase 3: R-sweep  (σ={sigma_injected:.3f} m, {traj}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    rng = np.random.default_rng(42)
    ...
    print(f"  True σ²= {true_var:.2e}  (expect optimal R near this value)")
    print(f"  {'R':>10} {'ekf_com':>12} {'note':>20}")
    ...
    print(f"\n  Best R={best_R:.1e}  (true σ²={true_var:.1e})")
...
def _make_nees_capturing_wrapper(nees_records):
    ...
    def _nees_step(x_quat_np, P_exp, simulator, ctrl, F_exp, H_np, z_exp,
        ...
        if have_measurement and z_exp is not None:
            # Recompute S and innovation (same math as _ekf_step)
            try:
...
def phase4_diagnostics(model_path, dataset_root, traj, device,
                        process_noise=1e-6, measurement_noise=1e-1,
                        save_plots=False):
    """Log and plot per-step Kalman diagnostics + NEES consistency test."""
    print(f"\n=== Phase 4: Per-step diagnostics ({traj}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    n_rods = len(sim.robot.rigid_bodies)
    ...
    # Chi-square 5%/95% bounds for meas_dim dofs
    nsteps = len(nees_records)
    avg_nees_per_step = mean_nees
...
def phase5_gating(model_path, dataset_root, traj, device,
    ...
    """Inject +1m position outlier at step outlier_step; gated flag must be True."""
    print(f"\n=== Phase 5: Gating test (outlier +{outlier_magnitude}m at step {outlier_step}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    n_rods = len(sim.robot.rigid_bodies)
    start = _make_start_state(gt_data, n_rods, device)
    ...
    # The outlier is at step outlier_step in gt_data, which corresponds to
    # the (outlier_step)-th call to _ekf_step (0-indexed)
    target_idx = outlier_step - 1  # extra_data index (step k uses gt_data[k+1])
...
def phase5_missing_measurements(model_path, dataset_root, traj, device,
                                  dropout=0.2, seed=42):
    """Drop measurements at random (dropout fraction); EKF should still beat raw GNN."""
    print(f"\n=== Phase 5: Missing measurements (dropout={dropout:.0%}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    n_rods = len(sim.robot.rigid_bodies)
    start = _make_start_state(gt_data, n_rods, device)
    ...
    # Create sparse gt_data: replace dropped steps with a sentinel
    gt_sparse = deepcopy(gt_data)
    dropped = 0
    for i in range(1, len(gt_sparse)):
        if rng.random() < dropout:
            gt_sparse[i] = None  # will be handled by patched step
            dropped += 1

    print(f"  Dropped {dropped}/{len(gt_data)-1} measurement steps")

    # Patch run_ekf_rollout to skip None measurements
    _orig_run = run_ekf_rollout.__wrapped__ if hasattr(run_ekf_rollout, "__wrapped__") else None

    # We can't patch run_ekf_rollout easily since it uses gt_data[k+1] internally.
    # Instead we run a custom loop using OnlineEKF.
    ...
    print(f"  raw_com={raw_com:.4e}  ekf_com(sparse)={ekf_com:.4e}  ratio={ratio:.3f}")
    print(f"  {status} — EKF {'beats' if ratio < 1.0 else 'does NOT beat'} raw GNN with {dropout:.0%} dropout")
    return ratio < 1.0


def phase5_jac_sweep(model_path, dataset_root, traj, device,
                      jac_periods=None):
    """Sweep jac_update_period; error should degrade gracefully."""
    if jac_periods is None:
        jac_periods = [1, 5, 10, 20]

    print(f"\n=== Phase 5: Jacobian update period sweep ({traj}) ===")
    data_dir = Path(dataset_root) / traj
    sim, gt_data, extra_data = _load_assets(model_path, data_dir, device)
    n_rods = len(sim.robot.rigid_bodies)
    start = _make_start_state(gt_data, n_rods, device)

    print(f"  {'jac_period':>12} {'ekf_com':>12} {'ratio_vs_J1':>14}")
    base_err = None
    prev_err = None
    all_graceful = True
    ...
    for J in jac_periods:
        ...
        # Graceful degradation: each doubling of period should not more than 10× error
...
def phase5_streaming_parity(model_path, dataset_root, traj, device,
                              n_steps=40, tol=1e-5):
    """OnlineEKF.step() state-by-state must match run_ekf_rollout (< tol m)."""
    print(f"\n=== Phase 5: Streaming vs batch parity ({traj}) ===")
    data_dir = Path(dataset_root) / traj
    ...
    online = OnlineEKF(
        ...
        dataset_idx_val=9,
        jacobian_update_interval=5,
        ema_alpha=1.0,  # disable EMA so output == current state
    )
    online.initialize(start.clone())
    ...
    for k in range(n):
        ...
        # online_out is exp-map (1,36,1); frames[k+1]['state'] is also exp-map (1,36,1)
...
# ---------------------------------------------------------------------------
# Phase 6: Cross-trajectory summary table
# ---------------------------------------------------------------------------

def phase6_cross_traj(rows):
    """Print mean/std of ekf/raw ratio across trajectories."""
    if not rows:
        return
    ratios = [r["ratio"] for r in rows]
    print("\n=== Phase 6: Cross-trajectory generalization ===")
    print(f"  Trajectories evaluated: {len(rows)}")
    print(f"  EKF/raw ratio  mean={np.mean(ratios):.3f}  std={np.std(ratios):.3f}")
...
# ---------------------------------------------------------------------------
# Optional plot helpers (require matplotlib)
# ---------------------------------------------------------------------------
...
def _plot_noise_sweep(sigmas, ekf_coms, raw_com, traj):
    ...
    ax.plot(sigmas, ekf_coms, "o-", label="EKF COM error")
    ax.axhline(raw_com, color="r", linestyle="--", label="Raw GNN COM error")
    ax.set_xlabel("Injected noise σ_pos (m)")
    ax.set_ylabel("COM MSE (m²)")
    ax.set_title(f"Noise sweep — {traj}")
    ax.legend()
    fig.tight_layout()
    out = f"noise_sweep_{traj}.png"
    fig.savefig(out)
    print(f"  Saved: {out}")
    plt.close(fig)
...
def _plot_diagnostics(innov_norms, nees_vals, traces, traj):
    ...
    axes[0].set_title(f"MEKF diagnostics — {traj}")

    axes[1].plot(steps, nees_vals)
    axes[1].axhline(np.mean(nees_vals), color="r", linestyle="--", label=f"mean={np.mean(nees_vals):.1f}")
...
def main():
    parser = argparse.ArgumentParser(description="MEKF Phase 3–5 evaluation")
